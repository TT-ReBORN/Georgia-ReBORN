...
/** @global @type {string} */
const libRequiredVersionStr = '1.5.2';

function lib_is_compatible(libRequiredVersionStr) {
	const requiredVersion = libRequiredVersionStr.split('.');
...
if (!lib_is_compatible(libRequiredVersionStr)) fb.ShowPopupMessage(`Library Tree requires v${libRequiredVersionStr}. Current component version is v${utils.Version}.`);

/** @global @type {ActiveXObject} */
const libDoc = new ActiveXObject('htmlfile');
/** @global @type {ActiveXObject} */
const libFSO = new ActiveXObject('Scripting.FileSystemObject');
/** @global @type {FbTooltip} */
const libTooltip = window.Tooltip;
/** @global @type {ActiveXObject} */
const libWshShell = new ActiveXObject('WScript.Shell');
...
class LibHelpers {
	...
	create(fo) {
		try {
			if (!this.folder(fo)) libFSO.CreateFolder(fo);
		} catch (e) {}
	}
	...
	file(f) {
		return typeof f === 'string' && libFSO.FileExists(f);
	}

	folder(fo) {
		return typeof fo === 'string' && libFSO.FolderExists(fo);
	}
	...
	getClipboardData() {
		try {
			...
			try {
				return libDoc.parentWindow.clipboardData.getData('Text');
			} catch (e) {
				return null;
			}
		}
	...
	getDpi() {
		let dpi = 120;
		try {
			dpi = libWshShell.RegRead('HKCU\\Control Panel\\Desktop\\WindowMetrics\\AppliedDPI');
		} catch (e) {}
	...
	setClipboardData(n) {
		try {
			...
			try {
				libDoc.parentWindow.clipboardData.setData('Text', n);
			} catch (e) {
	...
	wshPopup(prompt, caption) {
		try {
			const ns = libWshShell.Popup(prompt, 0, caption, 1);
			if (ns == 1) return true;
			return false;
		} catch (e) {
			return true;
...
/**
 * The instance of `LibHelpers` class for library helper operations.
 * @typedef {LibHelpers}
 * @global
 */
const $Lib = new LibHelpers();
