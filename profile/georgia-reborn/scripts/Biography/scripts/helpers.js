...
if (!is_compatible(requiredVersionStr)) fb.ShowPopupMessage(`Biography requires v${requiredVersionStr}. Current component version is v${utils.Version}.`);

const docBio = new ActiveXObject('htmlfile');
const fsoBio = new ActiveXObject('Scripting.FileSystemObject');
const tooltipBio = window.Tooltip;
const WshShellBio = new ActiveXObject('WScript.Shell');
...
class Helpers {
	...
	create(fo) {
		try {
			if (!this.folder(fo)) fsoBio.CreateFolder(fo);
		} catch (e) {}
	}
	...
	eval(n, focus, ignoreLock) {
		...
		if (panelBio.isRadio(focus)) return tfo.Eval();
	...
	file(f) {
		return typeof f === 'string' && fsoBio.FileExists(f);
	}

	folder(fo) {
		return typeof fo === 'string' && fsoBio.FolderExists(fo);
	}
	...
	getClipboardData() {
		try {
			...
			try {
				return docBio.parentWindow.clipboardData.getData('Text');
			} catch (e) {
				return null;
			}
		}
	...
	getDpi() {
		let dpi = 120;
		try {
			dpi = WshShellBio.RegRead('HKCU\\Control Panel\\Desktop\\WindowMetrics\\AppliedDPI');
		} catch (e) {}
	...
	handle(focus, ignoreLock) {
		return !panelBio.lock || ignoreLock ? fb.IsPlaying && !focus ? fb.GetNowPlaying() : fb.GetFocusItem() : panelBio.lockHandle;
	}
	...
	lastAccessed(file) {
		try {
			return Date.parse(fsoBio.GetFile(file).DateLastAccessed);
		} catch (e) {}
	}

	lastModified(file) {
		try {
			return Date.parse(fsoBio.GetFile(file).DateLastModified);
		} catch (e) {}
	}
	...
	run(c, w) {
		try {
			w === undefined ? WshShellBio.Run(c) : WshShellBio.Run(c, w);
			return true;
		} catch (e) {
			return false;
		}
	...
	wshPopup(prompt, caption) {
		try {
			const ns = WshShellBio.Popup(prompt, 0, caption, 1);
			if (ns == 1) return true;
			return false;
		} catch (e) {
			return true;
