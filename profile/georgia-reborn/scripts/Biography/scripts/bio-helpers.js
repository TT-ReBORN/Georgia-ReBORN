...
/** @global @type {string} */
const bioRequiredVersionStr = '1.5.2';

function lib_is_compatible(bioRequiredVersionStr) {
	const requiredVersion = bioRequiredVersionStr.split('.');
...
if (!lib_is_compatible(bioRequiredVersionStr)) fb.ShowPopupMessage(`Biography requires v${bioRequiredVersionStr}. Current component version is v${utils.Version}.`);

/** @global @type {ActiveXObject} */
const bioDoc = new ActiveXObject('htmlfile');
/** @global @type {ActiveXObject} */
const bioFSO = new ActiveXObject('Scripting.FileSystemObject');
/** @global @type {FbTooltip} */
const bioTooltip = window.Tooltip;
/** @global @type {ActiveXObject} */
const bioWshShell = new ActiveXObject('WScript.Shell');
...
class BioHelpers {
	...
	browser(c, b) {
		if (!this.run(c)) fb.ShowPopupMessage(b ? 'Unable to launch your default browser.' : 'Unable to open windows explorer', 'Biography');
	}
	...
	create(fo) {
		try {
			if (!this.folder(fo)) bioFSO.CreateFolder(fo);
		} catch (e) {}
	}
	...
	eval(n, focus, ignoreLock) {
		...
		if (bio.panel.isRadio(focus)) return tfo.Eval();
		const handle = this.handle(focus, ignoreLock);
		return handle ? tfo.EvalWithMetadb(handle) : '';
	}

	file(f) {
		return typeof f === 'string' && bioFSO.FileExists(f);
	}

	folder(fo) {
		return typeof fo === 'string' && bioFSO.FolderExists(fo);
	}
	...
	getClipboardData() {
		try {
			...
			try {
				return bioDoc.parentWindow.clipboardData.getData('Text');
			} catch (e) {
				return null;
			}
		}
	...
	getDpi() {
		let dpi = 120;
		try {
			dpi = bioWshShell.RegRead('HKCU\\Control Panel\\Desktop\\WindowMetrics\\AppliedDPI');
		} catch (e) {}
	...
	handle(focus, ignoreLock) {
		return !bio.panel.lock || ignoreLock ? fb.IsPlaying && !focus ? fb.GetNowPlaying() : fb.GetFocusItem() : bio.panel.lockHandle;
	}
	...
	htmlParse(n, prop, match, func) {
		...
		switch (sw) {
			case 0:
				while (i < ln) {
					if (n[i][prop] == match && func(n[i]) === true) break;
					i++;
				}
				break;
			case 1:
	...
	lastAccessed(file) {
		try {
			return Date.parse(bioFSO.GetFile(file).DateLastAccessed);
		} catch (e) {}
	}

	lastModified(file) {
		try {
			return Date.parse(bioFSO.GetFile(file).DateLastModified);
		} catch (e) {}
	}
	...
	run(c, w) {
		try {
			w === undefined ? bioWshShell.Run(c) : bioWshShell.Run(c, w);
			return true;
		} catch (e) {
			return false;
		}
	...
	trace(message, n) {
		if (!bioCfg.showConsoleMessages) return;
		console.log(`Biography${n ? ' Server' : ''}: ${message}`);
	}
	...
	wshPopup(prompt, caption) {
		try {
			const ns = bioWshShell.Popup(prompt, 0, caption, 1);
			if (ns == 1) return true;
			return false;
		} catch (e) {
			return true;
...
/**
 * The instance of `BioHelpers` class for biography helper operations.
 * @typedef {BioHelpers}
 * @global
 */
const $Bio = new BioHelpers();
