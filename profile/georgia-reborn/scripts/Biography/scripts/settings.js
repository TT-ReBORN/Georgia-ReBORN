...
class SettingsBio {
	...
	init(settings) {
		...
		this.autoCacheTime = $Bio.clamp(this.autoCacheTime, 0, 4);
	...
	validate(item) {
		if (!$Bio.isArray(item) || item.length !== 4 || typeof item[3] !== 'string') {
	...
	set(key, new_value) {
		...
		$Bio.save(this.bio, JSON.stringify($Bio.sortKeys(this.cfg), null, 3), true);
	}
	...
	checkTempFiles() {
		if (!this.autoCache || !this.autoCacheTime || !$Bio.server) return;
		const tmpFileTimer = setTimeout(() => {
			if (!$Bio.server) {
			...
			const paths = pths.filter(v => $Bio.folder(this.cachePath + v));
				let i = 0;
				const fileTimer = setInterval(() => {
					if ($Bio.server && i < paths.length) {
	...
	checkTempImages() {
		if (!this.autoCache || !this.autoCacheTime || !$Bio.server) return;
		const tmpImageTimer = setTimeout(() => {
			if (!$Bio.server) {
	...
	open() {
		const ok_callback = (new_ppt, new_cfg, type, new_dialogWindow) => {
			...
			if (new_cfg) {
				...
				this.updateCfg($Bio.jsonParse(new_cfg, {}));
			...
			switch (type) {
				...
				case 'empty': {
					try {
						...
						$Bio.trace('unable to empty photo recycler: can be emptied using windows explorer');
					}
					break;
				}
				case 'open':
					$Bio.browser(this.photoRecycler, false);
					break;
			}
		};
	...
	parse() {
		...
		this.fuzzyMatchTrack = $Bio.clamp(this.fuzzyMatchTrack, 0, 100);
		this.fuzzyMatchComposition = $Bio.clamp(this.fuzzyMatchComposition, 0, 100);
		...
		if (this.supCache) { // supplemental
			...
			const arr1 = replace.map(v => v.trim() || 'biography');
			replace = this.supReplace.split('|');
			const arr2 = replace.map(v => v.trim() || 'biography\\bio_supplemental');
			bioRevItems.forEach(v => {
				this.sup[v] = this[v];
...
let settingsBio = [
	...
	['Album Review [Allmusic] Folder', '%storage_folder%\\biography-cache\\review\\allmusic\\$lower($cut(%BIO_ALBUMARTIST%,1))', 'text', 'foAmRev'],
	['Biography [Allmusic] Folder', '%storage_folder%\\biography-cache\\biography\\allmusic\\$lower($cut(%BIO_ARTIST%,1))', 'text', 'foAmBio'],
	['Album Review [Lastfm] Folder', '%storage_folder%\\biography-cache\\review\\lastfm\\$lower($cut(%BIO_ALBUMARTIST%,1))', 'text', 'foLfmRev'],
	...
	['Supplemental Cache [Use Find>Replace on SAVE paths]', 'biography>biography\\bio_supplemental', 'text', 'supFo'],
	['Supplemental Cache Find', 'biography', 'text', 'supFind'],
	['Supplemental Cache Replace', 'biography\\bio_supplemental', 'text', 'supReplace'],
...
class radioStreamParser {
	static getStreamInfo(focus, ignoreLock) {
		...
		// adjust format
		artist = artist.toLowerCase(); // toLowerCase() stops all uppercase being treated as abbreviation by $Bio.titlecase
		artist = $Bio.titlecase(artist);
		//console.log('formatted artist', artist);

		title = title.toLowerCase();
		title = $Bio.titlecase(title);
