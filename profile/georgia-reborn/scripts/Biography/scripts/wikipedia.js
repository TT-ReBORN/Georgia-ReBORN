...
class DldWikipedia {
	...
	search(p_searchItem, p_artist, p_title, p_album, p_recording, p_type, p_fo, p_pth, p_focus, p_force) {
		...
		if (this.init) {
			...
			this.title = p_title;
			this.name = this.artist + (this.title ? ` - ${$Bio.titlecase(this.title)}` : '');
	...
	analyse() {
		switch (this.searchItem) {
			case 0: // getArtistMbid
				switch (this.aridType) {
					case 0: case 1: { // recording or release-group match
						const data = $Bio.jsonParse(this.xmlhttp.responseText, [], 'get', !this.aridType ? 'recordings' : 'release-groups');
						if (!data.length) {
					...
					case 2: { // artist name match: type 0: only accepted if single name match: others try wikipedia direct as can validate then
						const data = $Bio.jsonParse(this.xmlhttp.responseText, [], 'get', 'artists');
						if (!data.length) {
							if (this.doFallbackSearch()) return;
							if (this.type || !$Bio.file(this.pth)) return $Bio.trace(`wikipedia: ${this.artist}: name not matched at musicbrainz`, true);
						}
						const artist = $Bio.strip(this.artist);
						const aliases = [];
						let items = [];
						...
						if (!this.ar_mbid) {
							...
							if (this.type || !$Bio.file(this.pth)) return $Bio.trace(`wikipedia: ${this.artist}${items.length > 1 || aliases.length > 1 ? `: unable to disambiguate multiple artists of same name: discriminators${!this.lookUpArt ? ', album name or track title, not matched' : ' N/A for menu look ups'}` : ': name not matched at musicbrainz'}`, true); // type 0 only
						}
			...
			case 1: { // extractMbArtistLinks
				let data = $Bio.jsonParse(this.xmlhttp.responseText, []);
				...
				if (!this.type) {
					...
					$Bio.take(this.genres, 5);
					this.genres = this.genres.map(v => $Bio.titlecase(v.name));
				}

				let countryCode = '';
				['area.iso-3166-1-codes.0', 'area.iso-3166-2-codes.0', 'begin-area.iso-3166-1-codes.0', 'begin-area.iso-3166-2-codes.0'].some(v => countryCode = $Bio.jsonParse(this.xmlhttp.responseText, '', 'get', v));
				this.saveCountryCode(countryCode, this.force);

				data = $Bio.jsonParse(this.xmlhttp.responseText, [], 'get', 'relations');
				this.wikidata = '';
				this.wikititle = '';
				let wikidataCount = 0;
				...
				if (wikidataCount == 1) {
					...
					data.some(v => {
							if (v.type == 'wikidata' && (!v['source-credit'] || $Bio.strip(v['source-credit']).includes($Bio.strip(this.artist)))) {
							return this.wikidata = v.url.resource;
						}
					});
				}
			...
			case 2: { // getItemMbid
				const data = $Bio.jsonParse(this.xmlhttp.responseText, [], 'get', this.type != 2 && this.type != 4 ? 'release-groups' : 'works');
				const list = [];
				const filteredList = []; // check work disambiguation
				const ar = $Bio.strip(this.related_artist || this.artist);
				data.forEach(v => {
					...
					if (this.type == 2 || this.type == 4) {
						...
						if (levenshtein.get($Bio.strip(nm), ar) > 0.8 || levenshtein.get($Bio.strip(disamb), ar) > 0.8) filteredList.push({ artist: this.artist, date: Date.parse(v['first-release-date']) || 0, rg_mbid: v.id, title: v.title });
					}
				});
				$Bio.sort(list, 'date', 'num');
				$Bio.sort(list, 'type');
				let i = -1;
				if (this.type == 2 || this.type == 4) {
			...
			case 3: { // extractMbItemLinks
				let data;
				if (this.type != 4) {
					data = $Bio.jsonParse(this.xmlhttp.responseText, []);
					this.genres = data.genres.filter(v => v.count > 0).sort((a, b) => b.count - a.count);
					$Bio.take(this.genres, 5);
					this.genres = this.genres.map(v => $Bio.titlecase(v.name));
				}
			...
			case 6: { // artistWorks
				const response = $Bio.jsonParse(this.xmlhttp.responseText, [], 'get', 'works');
				if (!this.offset) this.releases = $Bio.jsonParse(this.xmlhttp.responseText, 0, 'get', 'work-count');
				const list = [];
				response.forEach(v => {
				...
				if (i == -1) {
					...
					if (this.releases > this.offset) {
						...
						if (this.doFallbackSearch()) return;
						return $Bio.trace(`wikipedia: ${this.name}: not found`, true);
					}
				}
			...
			case 7: { // directSearch
				const data = $Bio.jsonParse(this.xmlhttp.responseText, [], 'get', 'query.search');
	...
	getRgMbid() {
		...
		this.rg_mbid = $Bio.eval(`$trim($if3(%musicbrainz_${type1}id%,%musicBrainz ${type2} id%,))`, this.focus);
	...
	save() {
		...
		if (this.type < 3) {
			if (!this.wiki) {
				if (this.type || !$Bio.file(this.pth)) $Bio.trace(`wikipedia: ${this.title || this.artist}: not found`, true);
				return;
			}
			if (this.fo) {
...
class InfoboxBio {
	...
	getComposer(item, list, site) {
		const n = [];
		const m = this.find(item, list, site);
		if (!m) return n;
	...
	getGenre(item, list, site) {
		...
		if (site == 'it' && m.length > 1) return m.map(v => $Bio.titlecase(v));
		...
		if (/{{start\s?date\|/i.test(m[0])) {
			...
			if (dates) {
				const map = dates.map(v => {
					...
					let date = d1.join('-');
					date = new Date(date);
					const d2 = this.isValidDate(date) ? ['', this.yyyy.format(date), this.mm_yyyy.format(date), this.date.format(date)][type] : '';
					return { date: d1, d: d2 };
				});
				d = m[0];
				dates.forEach((v, i) => {
