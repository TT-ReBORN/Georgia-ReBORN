...
class ServerBio {
	...
	download(force, art, alb, name) {
		...
		this.getRev(force, art, alb, force == 2);
	}
	...
	getObjKeyValue(obj, searchKey, results = []) {
		...
		Object.keys(obj).forEach(key => {
			const value = obj[key];
			if (key === searchKey && !$Bio.isObject(value)) {
				r.push(value);
			} else if ($Bio.isObject(value)) {
	...
	match(p_a, p_release, list, type, wikipedia, force) {
		...
		a = $Bio.removeDiacritics(this.tidy(p_a, true));
		rel = $Bio.removeDiacritics(this.tidy(p_release, true));
		for (i = 0; i < list.length; i++) { // strip match
			list[i].rel = $Bio.removeDiacritics(this.tidy(list[i].title || 'N/A', true));
	...
	urlDone(n) {
		...
		const done = !!this.urlRequested[n];
		if (!done) {
