class Buttons {
	...
	move(x, y) {
		...
		if (hover_btn && !(hover_btn.down && hover_btn.type < 4)) {
			hover_btn.cs('hover');
			if (this.tooltipLib.show && hover_btn.tiptext) hover_btn.tt.show(hover_btn.tiptext());
			this.transition.start();
		}
		this.cur = hover_btn;
		return this.cur;
				...
				case 2:
					switch (true) {
						case this.vertical:
							...
							break;
						case !this.vertical:
							this.btns.scrollUp = new Btn(ui.x + this.scr.xLeft1, this.scr.y1, ui.sbar.but_h, ui.sbar.but_h, 3, '', '', '', {
								normal: 9,
								hover: 10,
								down: 11
							}, ppt.sbarShow == 1 && sbar.narrow.show || sbar.scrollable_lines < 1, () => sbar.but(1), '', '', false, 'scrollUp');
