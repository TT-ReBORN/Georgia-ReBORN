class Panel {
	...
	calcText() {
		...
		switch (true) {
			...
			case !ppt.settingsShow:
				this.filter.x = ui.x + ui.w - this.filter.w;
				break;
			case !ppt.filterShow:
	...
	forcePaint() {
		window.RepaintRect(ui.x, ui.y, ui.w, ui.h - this.paint_y + 1, true);
	}
	...
	getFilters() {
		...
		for (let i = pptNo + 1; i < 100; i++) {
			nm = ppt.get(`Filter ${$Lib.padNumber(i, 2)}: Name // Query`);
			if (nm) {
	...
	getViews() {
		let pt = [
			...
			["View 03: Name // Pattern", "View by Composer // [%<composer>%]|%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%"],
			['View 04: Name // Pattern', 'View by Album Artist // %album artist%|%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%'],
			['View 05: Name // Pattern', "View by Album Artist - Album // [%album artist% - ]['['%date%']' ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%"],
			['View 06: Name // Pattern', "View by Album // %album%[ '['%album artist%']']|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%"],
			['View 07: Name // Pattern', 'View by Genre // %<genre>%|[%album artist% - ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%'],
			['View 08: Name // Pattern', 'View by Year // %date%|[%album artist% - ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%']
		];
		let grps = [];
		...
		let nm = '';
		for (let i = pptNo + 1; i < 100; i++) {
			nm = ppt.get(`View ${$Lib.padNumber(i, 2)}: Name // Pattern`);
			if (nm) {
		...
		const ok_callback = (new_cfg, new_ppt, type, new_cfgWindow) => {
			if (new_cfg) {
				...
				cfg[0].forEach((v, i) => {
					...
					ppt.set(v.type != 'Pattern Not Configurable' ? `View ${$Lib.padNumber(i + 2, 2)}: Name // Pattern` : `View 01: Name // Pattern`, nm);
				});
				i = cfg[1].length;
				while (i--)
				...
				cfg[1].forEach((v, i) => {
					const nm = v.type ? v.name + (v.menu ? ' // ' : ' /hide/ ') + v.type : null;
					ppt.set(v.type != 'Button Name' ? `Filter ${$Lib.padNumber(i + 2, 2)}: Name // Query` : `Filter 01: Name // Query`, nm);
				});
	...
	on_size(fontChanged) {
		...
		this.search.h = ui.style.topBarShow ? ui.row.h + (!ui.id.local ? ln_sp * 2 + ui.sz.margin + scaleForDisplay(7) : 0) : ui.sz.margin;
		this.search.sp = this.search.h - ln_sp;
		let sp = ui.h - this.search.h - (ui.style.topBarShow ? ui.sz.margin / 2 : ui.sz.margin);
				...
				switch (i) {
					case 0: {
						...
						// 	if (confirmed) {
								...
								if (pref.libraryThumbnailSize === 'auto') ppt.thumbNailSize = 1;
					...
					case 1: {
						...
						// 	if (confirmed) {
								...
								panel.imgView = ppt.albumArtShow = false;
								ppt.albumArtLabelType = 1;
								if (pref.libraryThumbnailSize === 'auto') ppt.thumbNailSize = 1; // In default the same
								ppt.artId = 0;
								ppt.albumArtFlowMode = false; // In default does not exist
								this.load();
						// 	}
					...
					case 4: {
						// const continue_confirmation = (status, confirmed) => {
						// 	if (confirmed) {
								pref.libraryDesign = 'listView_albumCovers';
								pref.libraryLayout = 'normal_width';
								if (pref.libraryThumbnailSize === 'auto') ppt.thumbNailSize = 0;
								ppt.highLightNowplaying = true; // In default does not exist
								ppt.zoomNode = 100; // In default does not exist
								ppt.nodeStyle = 4; // In default does not exist
					...
					case 5: {
						// const continue_confirmation = (status, confirmed) => {
						// 	if (confirmed) {
								pref.libraryDesign = 'listView_artistPhotos';
								pref.libraryLayout = 'normal_width';
								if (pref.libraryThumbnailSize === 'auto') ppt.thumbNailSize = 0;
								ppt.highLightNowplaying = true; // In default does not exist
								ppt.zoomNode = 100; // In default does not exist
								...
								ppt.albumArtLabelType = 2;
								ppt.imgStyleFront = 1;
								ppt.itemOverlayType = 0; // ppt.itemOverlayType = 2;
								ppt.artId = 4; // ppt.artId = 0;
								ppt.albumArtGrpLevel = 0;
								this.load();
						// 	}
						// }
					...
					case 6: {
						// const continue_confirmation = (status, confirmed) => {
						// 	if (confirmed) {
								pref.libraryDesign = 'albumCovers';
								pref.libraryLayout = 'normal_width';
								ppt.highLightNowplaying = true; // In default does not exist
								ppt.zoomNode = 100; // In default does not exist
								ppt.nodeStyle = 4; // In default does not exist
					...
					case 7: {
						...
						// 	if (confirmed) {
								...
								ppt.zoomNode = 100; // In default does not exist
								ppt.countsRight = true;
								ppt.nodeStyle = 4; // ppt.nodeStyle = 2;
								ppt.inlineRoot = true;
								ppt.autoCollapse = false;
								ppt.treeAutoExpandSingle = false;
								ppt.treeListView = false;
					...
					case 9: {
						...
						// 	}
						// }
						// popUpBox.confirm('Quick Setup: Georgia-ReBORN Style', msg, 'Yes', 'No', continue_confirmation);
						break;
					}
				}
				break;
	...
	setTopBar() {
		let sz = Math.round(12 * $Lib.scale * this.zoomFilter);
		let mod = 0;
		if (sz > 15) mod = (sz % 2) - 1;
		this.filter.font = gdi.Font('Segoe UI', this.zoomFilter > 1.05 ? Math.floor(ppt.baseFontSize /*11 * $Lib.scale * this.zoomFilter*/) : Math.max(ppt.baseFontSize /*11 * $Lib.scale * this.zoomFilter*/, 9), 1);
		this.settings.font = gdi.Font('Segoe UI Symbol', ppt.baseFontSize /*sz + mod*/, 0);
		this.settings.icon = '\uE10C';
	...
	treePaint() {
		window.RepaintRect(ui.x, ui.y + this.paint_y - scaleForDisplay(10), ui.w, ui.h - this.paint_y + scaleForDisplay(10));
	}
