class ResizeHandler {
	constructor() {
		this.down = false;
		this.editorFont = gdi.Font('Segoe UI', 15 * $Bio.scale, 1);
		this.focus = true;
	...
	drawEd(gr) {
		if (vkBio.k('ctrl') && this.focus && panelBio.m.y != -1 || panelBio.style.new) {
			...
			if (!pptBio.text_only && !pptBio.img_only && txt.text) {
				if (pptBio.style > 3) {
	...
	filmMove(x, y) {
		...
		switch (pptBio.filmStripPos) {
			case 0:
				...
				pptBio.filmStripSize = $Bio.clamp(pptBio.filmStripSize, 0.02, filmStrip.max_sz / panelBio.h);
				break;
			case 1:
				pptBio.filmStripSize = (pptBio.filmStripSize * panelBio.w + this.x_start - x) / panelBio.w;
				pptBio.filmStripSize = $Bio.clamp(pptBio.filmStripSize, 0.02, filmStrip.max_sz / panelBio.w);
				break;
			case 2:
	...
	lbtn_up() {
		...
		img.mask.reset = true;
		if (pptBio.style > 3) {
			const obj = pptBio.style == 4 ? panelBio.style.overlay : panelBio.style.free[pptBio.style - 5];
	...
	imgMove(x, y) {
		if (!this.focus || pptBio.img_only || !txt.text || pptBio.text_only) return;
		switch (true) {
			case pptBio.style > 3: {
				...
				let imT = Math.round(panelBio.im.t * panelBio.h) + panelBio.filmStripSize.t;
				let imB = Math.round(panelBio.im.b * panelBio.h) + panelBio.filmStripSize.b;
				let imL = Math.round(panelBio.im.l * panelBio.w) + panelBio.filmStripSize.l;
				let imR = Math.round(panelBio.im.r * panelBio.w) + panelBio.filmStripSize.r;
				switch (this.si) {
					case 'top':
						if (y > panelBio.h - imB - 30) break;
						panelBio.im.t = $Bio.clamp((y - panelBio.filmStripSize.t) / panelBio.h, 0, 1);
						break;
					case 'nw':
						if (y < panelBio.h - imB - 30) panelBio.im.t = $Bio.clamp((y - panelBio.filmStripSize.t) / panelBio.h, 0, 1);
						if (x > panelBio.w - imR - 30) break;
						panelBio.im.l = $Bio.clamp((x - panelBio.filmStripSize.l) / panelBio.w, 0, 1);
						break;
					case 'ne':
						if (y < panelBio.h - imB - 30) panelBio.im.t = $Bio.clamp((y - panelBio.filmStripSize.t) / panelBio.h, 0, 1);
						if (x < imL + 30) break;
						panelBio.im.r = $Bio.clamp((panelBio.w - x - panelBio.filmStripSize.r) / panelBio.w, 0, 1);
						break;
					case 'left':
						if (x > panelBio.w - imR - 30) break;
						panelBio.im.l = $Bio.clamp((x - panelBio.filmStripSize.l) / panelBio.w, 0, 1);
					...
					case 'bottom':
						if (y < imT + 30) break;
						panelBio.im.b = $Bio.clamp((panelBio.h - y - panelBio.filmStripSize.b) / panelBio.h, 0, 1);
						break;
					case 'sw':
						if (x < panelBio.w - imR - 30) panelBio.im.l = $Bio.clamp((x - panelBio.filmStripSize.l) / panelBio.w, 0, 1);
						if (y < imT + 30) break;
						panelBio.im.b = $Bio.clamp((panelBio.h - y - panelBio.filmStripSize.b) / panelBio.h, 0, 1);
						break;
					case 'se':
						if (y > imT + 30) panelBio.im.b = $Bio.clamp((panelBio.h - y - panelBio.filmStripSize.b) / panelBio.h, 0, 1);
						if (x < imL + 30) break;
						panelBio.im.r = $Bio.clamp((panelBio.w - x - panelBio.filmStripSize.r) / panelBio.w, 0, 1);
						break;
					case 'right':
						...
						break;
					case 'all':
						if (imT <= panelBio.filmStripSize.t && y - this.init_y < 0 || imB <= panelBio.filmStripSize.b && y - this.init_y > 0 || imL <= panelBio.filmStripSize.l && x - this.init_x < 0 || imR <= panelBio.filmStripSize.r && x - this.init_x > 0) break;
						imT += (y - this.y_init);
						panelBio.im.t = $Bio.clamp((imT - panelBio.filmStripSize.t) / panelBio.h, 0, 1);
						imB = panelBio.h - Math.max(imT, 0) - panelBio.ibox.h;
						panelBio.im.b = $Bio.clamp((imB - panelBio.filmStripSize.b) / panelBio.h, 0, 1);
						imL += (x - this.x_init);
						panelBio.im.l = $Bio.clamp((imL - panelBio.filmStripSize.l) / panelBio.w, 0, 1);
						imR = panelBio.w - Math.max(imL, 0) - panelBio.ibox.w;
						panelBio.im.r = $Bio.clamp((imR - panelBio.filmStripSize.r) / panelBio.w, 0, 1);
						break;
				}
				this.sizes(true);
				break;
			...
			case pptBio.style < 4: {
				...
				if (!this.down || !this.si) return;
				const ph = panelBio.h - panelBio.filmStripSize.t - panelBio.filmStripSize.b;
				const pw = panelBio.w - panelBio.filmStripSize.l - panelBio.filmStripSize.r;
				switch (pptBio.style) {
					case 0:
				...
				pptBio.rel_imgs = $Bio.clamp(pptBio.rel_imgs, 0.1, 0.9);
				this.sizes();
				break;
			}
		}
	...
	move(x, y) {
		if (pptBio.style < 4 || pptBio.img_only || !txt.text || pptBio.text_only || !vkBio.k('ctrl') || vkBio.k('alt') || !this.focus) return;
		if (!this.down) {
		...
		let txT = Math.round(panelBio.tx.t * panelBio.h) + panelBio.filmStripSize.t;
		let txB = Math.round(panelBio.tx.b * panelBio.h) + panelBio.filmStripSize.b;
		let txL = Math.round(panelBio.tx.l * panelBio.w) + panelBio.filmStripSize.l;
		let txR = Math.round(panelBio.tx.r * panelBio.w) + panelBio.filmStripSize.r;
		switch (this.st) {
			case 'top':
				if (y > panelBio.h - txB - panelBio.style.minH) break;
				panelBio.tx.t = $Bio.clamp((y - panelBio.filmStripSize.t) / panelBio.h, 0, 1);
				break;
			case 'nw':
				if (y < panelBio.h - txB - panelBio.style.minH) panelBio.tx.t = $Bio.clamp((y - panelBio.filmStripSize.t) / panelBio.h, 0, 1);
				if (x > panelBio.w - txR - 30) break;
				panelBio.tx.l = $Bio.clamp((x - panelBio.filmStripSize.l) / panelBio.w, 0, 1);
				break;
			case 'ne':
				if (y < panelBio.h - txB - panelBio.style.minH) panelBio.tx.t = $Bio.clamp((y - panelBio.filmStripSize.t) / panelBio.h, 0, 1);
				if (x < txL + 30) break;
				panelBio.tx.r = $Bio.clamp((panelBio.w - x - panelBio.filmStripSize.r) / panelBio.w, 0, 1);
				break;
			case 'left':
				if (x > panelBio.w - txR - 30) break;
				panelBio.tx.l = $Bio.clamp((x - panelBio.filmStripSize.l) / panelBio.w, 0, 1);
			...
			case 'bottom':
				...
				panelBio.tx.b = $Bio.clamp((panelBio.h - y - panelBio.filmStripSize.b) / panelBio.h, 0, 1);
				break;
			case 'sw':
				if (x < panelBio.w - txR - 30) panelBio.tx.l = $Bio.clamp((x - panelBio.filmStripSize.l) / panelBio.w, 0, 1);
				if (y < txT + panelBio.style.minH) break;
				panelBio.tx.b = $Bio.clamp((panelBio.h - y - panelBio.filmStripSize.b) / panelBio.h, 0, 1);
				break;
			case 'se':
				if (y > txT + panelBio.style.minH) panelBio.tx.b = $Bio.clamp((panelBio.h - y - panelBio.filmStripSize.b) / panelBio.h, 0, 1);
				if (x < txL + 30) break;
				panelBio.tx.r = $Bio.clamp((panelBio.w - x - panelBio.filmStripSize.r) / panelBio.w, 0, 1);
				break;
			case 'right':
				...
				break;
			case 'all':
				if (txT <= panelBio.filmStripSize.t && y - this.init_y < 0 || txB <= panelBio.filmStripSize.b && y - this.init_y > 0 || txL <= panelBio.filmStripSize.l && x - this.init_x < 0 || txR <= panelBio.filmStripSize.r && x - this.init_x > 0) break;
				txT += (y - this.init_y);
				panelBio.tx.t = $Bio.clamp((txT - panelBio.filmStripSize.t) / panelBio.h, 0, 1);
				txL += (x - this.init_x);
				panelBio.tx.l = $Bio.clamp((txL - panelBio.filmStripSize.l) / panelBio.w, 0, 1);
				txB = panelBio.h - Math.max(txT, 0) - panelBio.tbox.h;
				panelBio.tx.b = $Bio.clamp((txB - panelBio.filmStripSize.b) / panelBio.h, 0, 1);
				txR = panelBio.w - Math.max(txL, 0) - panelBio.tbox.w;
				panelBio.tx.r = $Bio.clamp((txR - panelBio.filmStripSize.r) / panelBio.w, 0, 1);
				break;
		}
		this.sizes(true);
		this.init_x = x;
