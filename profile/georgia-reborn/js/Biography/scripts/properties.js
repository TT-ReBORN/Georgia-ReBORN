...
let propertiesBio = [
	...
	['Cover Style [Dual Mode] Regular-0 Auto-Fill-1 Circular-2', 1, 'covStyleDual'],
	['Cover Style [Image Only] Regular-0 Auto-Fill-1 Circular-2', 1, 'covStyleImgOnly'],
	...
	['Filmstrip Autofit', false, 'filmStripAutofit'],
	['Filmstrip Cover Regular-0 Auto-Fill-1 Circular-2', 1, 'filmCoverStyle'],
	['Filmstrip Margin', 2, 'filmStripMargin'],
	['Filmstrip Photo Regular-0 Auto-Fill-1 Circular-2', 1, 'filmPhotoStyle'],
	['Filmstrip Pos', 3, 'filmStripPos'],
	['Filmstrip Size 0-1', 0.10, 'filmStripSize'],
	['Filmstrip Show', false, 'showFilmStrip'],
	...
	['Font Size Biography', scaleForDisplay(12), 'baseFontSizeBio'],
	['Freestyle Custom', JSON.stringify([]), 'styleFree'],

	['Heading', 2, 'heading'],
	['Heading Always Full Width', true, 'fullWidthHeading'],
	...
	['Heading Padding Bottom Line', 8, 'hdLinePad'],
	['Heading Padding', 0, 'hdPad'],
	['Heading Position', 0, 'hdRight'],
	['Heading Show Button Background', false, 'hdShowBtnBg'],
	...
	['Layout Margin Between Image & Text', 11, 'gap'],
	['Layout Margin Image Left', scaleForDisplay(40), 'borL'],
	['Layout Margin Image Right', scaleForDisplay(40), 'borR'],
	['Layout Margin Image Top', scaleForDisplay(70), 'borT'],
	['Layout Margin Image Bottom', 0, 'borB'],
	['Layout Margin Text Left', scaleForDisplay(40), 'textL'],
	['Layout Margin Text Right', scaleForDisplay(40), 'textR'],
	['Layout Margin Text Top', scaleForDisplay(70), 'textT'],
	['Layout Margin Text Bottom', 0, 'textB'],
	...
	['Photo Style [Dual Mode] Regular-0 Auto-Fill-1 Circular-2', 1, 'artStyleDual'],
	['Photo Style [Image Only] Regular-0 Auto-Fill-1 Circular-2', 1, 'artStyleImgOnly'],
	...
	['ScrollbarBio Height Prefer Full', false, 'sbarFullHeight'],
	...
	['ScrollbarBio Arrow Custom Icon', '\uE0A0', 'arrowSymbol'],
	['ScrollbarBio Arrow Custom Icon: Vertical Offset (%)', -24, 'sbarButPad'],
	['ScrollbarBio Arrow Width', Math.round(11 * $Bio.scale), 'sbarArrowWidth'],
	['ScrollbarBio Button Type', 0, 'sbarButType'],
	['ScrollbarBio Colour Grey-0 Blend-1', 1, 'sbarCol'],
	['ScrollbarBio Grip MinHeight', Math.round(20 * $Bio.scale), 'sbarGripHeight'],
	['ScrollbarBio Padding', 0, 'sbarPad'],
	['ScrollbarBio Narrow Bar Width (0 = Auto)', 0, 'narrowSbarWidth'],
	['ScrollbarBio Show', 1, 'sbarShow'],
	['ScrollbarBio Type Default-0 Styled-1 Windows-2', 0, 'sbarType'],
	['ScrollbarBio Width', Math.round(11 * $Bio.scale), 'sbarWidth'],
	['ScrollbarBio Width Bar', 11, 'sbarBase_w'],
	['ScrollbarBio Windows Metrics', false, 'sbarWinMetrics'],
...
class PanelPropertiesBioUpdate {
	getOrig(name) {
	...
	update(properties) {
		...
		names = [
			...
			' ScrollbarBio Arrow Custom: Icon: Vertical Offset %',
			' ScrollbarBio Narrow Bar Width 2-10 (0 = Default)',
			...
			'ADV.ScrollbarBio Height Always Full',
		...
		props = [
			...
			'ScrollbarBio Arrow Custom Icon: Vertical Offset (%)',
			'ScrollbarBio Narrow Bar Width (0 = Auto)',
			'Subheading [Source] Text Biography [AllMusic]: Heading|No Heading',
			'Subheading [Source] Text Biography [Last.fm]: Heading|No Heading',
			'Subheading [Source] Text Review [AllMusic]: Heading|No Heading',
			'Subheading [Source] Text Review [Last.fm]: Heading|No Heading',
			...
			'ScrollbarBio Height Prefer Full',
			'Image Filter Lastfm',
			'Image Seeker'
		];
		names.forEach((n, i) => {
		...
		let prop = ' ScrollbarBio Size';
		val = this.getOrig(prop);
		if (typeof val === 'string') {
			val = $Bio.split(val, 0);
			if (val.length == 8) {
				const sbarWidth = $Bio.clamp($Bio.value(val[1], 11, 0), 0, 400);
				this.setNew('ScrollbarBio Width', sbarWidth)
				this.setNew('ScrollbarBio Arrow Width', Math.min($Bio.value(val[3], 11, 0), sbarWidth, 400));
				this.setNew('ScrollbarBio Padding', $Bio.value(val[5], 0, 0));
				this.setNew('ScrollbarBio Grip MinHeight', $Bio.value(val[7], 12, 0));
			}
		}
		this.setOrigNull(prop);

		prop = ' ScrollbarBio Type Default-0 Styled-1 Themed-2';
		val = parseInt(this.getOrig(prop));
		if (typeof val !== 'number') val = 0;
		val = $Bio.clamp(val, 0, 2);
		this.setNew('ScrollbarBio Type Default-0 Styled-1 Windows-2', val);
		this.setOrigNull(prop);
		...
		prop = ' ScrollbarBio Arrow Custom: Icon // Examples';
		val = this.getOrig(prop);
		if (typeof val === 'string') {
			const ch = val.charAt();
			if (ch.length) this.setNew('ScrollbarBio Arrow Custom Icon', ch);
		}
		this.setOrigNull(prop);
		...
		if (typeof val === 'string') {
			val = $Bio.split(val, 0);
			if (val.length == 8) {
				this.setNew('Zoom Button Heading Size (%)', 100 + $Bio.value(val[5], 0, 0) * 10);
				this.setNew('Heading Padding Button', $Bio.value(val[7], 0, 0));
				this.setNew('Heading Padding Bottom Line', $Bio.value(val[3], 0, 0));
				this.setNew('Heading Padding', $Bio.value(val[1], 0, 0));
			}
		}
		this.setOrigNull(prop);
		...
		if (typeof val === 'string') {
			val = $Bio.split(val, 0);
			if (val.length == 8) {
				this.setNew('Menu Show Paste', $Bio.value(val[1], 1, 2));
				this.setNew('Menu Show Playlists', $Bio.value(val[3], 0, 2));
				this.setNew('Menu Show Tagger', $Bio.value(val[5], 1, 2));
				this.setNew('Menu Show Missing Data', $Bio.value(val[7], 0, 2));
			}
		}
		this.setOrigNull(prop);
		...
		if (typeof val === 'string') {
			val = $Bio.split(val, 0);
			if (val.length == 14) {
				this.setNew('Highlight Heading Button', $Bio.value(val[1], 0, 1));
				this.setNew('Highlight Heading Text', $Bio.value(val[3], 1, 1));
				this.setNew('Highlight Heading Line', $Bio.value(val[5], 1, 1));
				this.setNew('Highlight Overlay Border', $Bio.value(val[7], 1, 1));
				this.setNew('Highlight Rating Stars', $Bio.value(val[9], 1, 1));
				this.setNew('Highlight Subheadings', $Bio.value(val[11], 1, 1));
				this.setNew('Highlight Text', $Bio.value(val[13], 0, 1));
			}
		}
		this.setOrigNull(prop);
		...
		if (typeof val === 'string') {
			val = $Bio.split(val, 0);
			if (val.length == 8) {
				this.setNew('Heading Show Button Background', $Bio.value(val[1], 1, 1) === 1);
				this.setNew('Heading Button Show Label', $Bio.value(val[3], 1, 1) === 1 ? (useIcon ? 2 : 1) : 0);
				this.setNew('Heading Show Button Red Lfm', $Bio.value(val[5], 0, 1) === 1);
				this.setNew('Heading Show Title', $Bio.value(val[7], 1, 1) === 1);
			}
		}
		this.setOrigNull(prop);
		...
		if (typeof val === 'string') {
			val = $Bio.split(val, 0);
			if (val.length == 6) {
				this.setNew('Overlay Strength (%)', $Bio.value(val[1], 84.5, 0));
				this.setNew('Overlay Gradient (%)', $Bio.value(val[3], 10, 0));
				this.setNew('Overlay Border Width (px)', $Bio.value(val[5], 1, 0));
			}
		}
		this.setOrigNull(prop);
		...
		if (typeof val === 'string') {
			val = $Bio.split(val, 0);
			if (val.length == 6) {
				this.setNew('Reflection Strength (%)', $Bio.value(val[1], 14.5, 0));
				this.setNew('Reflection Size (%)', $Bio.value(val[3], 100, 0));
				this.setNew('Reflection Gradient (%)', $Bio.value(val[5], 10, 0));
			}
		}
		this.setOrigNull(prop);
		...
		if (typeof val === 'string') {
			val = $Bio.split(val, 0);
			if (val.length == 4) {
				this.setNew('Scroll Smooth Duration 0-5000 msec (Max)', $Bio.clamp($Bio.value(val[1], 500, 0), 0, 5000));
				this.setNew('Scroll Touch Flick Duration 0-5000 msec (Max)', $Bio.clamp($Bio.value(val[3], 3000, 0), 0, 5000));
			}
		}
		this.setOrigNull(prop);
		...
		if (pptBio.sbarShow === false) pptBio.sbarShow = 0;
		if (pptBio.sbarShow === true) pptBio.sbarShow = 1;
		if (pptBio.sourceStyle == 16) pptBio.sourceStyle = 4;
		if (pptBio.sourceStyle == 18) pptBio.sourceStyle = 5;
		if (pptBio.trackStyle == 16) pptBio.trackStyle = 4;
		if (pptBio.trackStyle == 18) pptBio.trackStyle = 5;
		if (pptBio.heading === false) pptBio.heading = 0;
		if (pptBio.heading === true) pptBio.heading = 2;
	}
}

const pptBio = new PanelPropertiesBio;
const pptUpdateBio = new PanelPropertiesBioUpdate;
pptUpdateBio.update(propertiesBio);
