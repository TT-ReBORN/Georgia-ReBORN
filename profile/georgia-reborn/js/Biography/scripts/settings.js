...
class SettingsBio {
	...
	init(settingsBio) {
		this.checkCfg(settingsBio);
		settingsBio.forEach(v => {
	...
	validate(item) {
		if (!$Bio.isArray(item) || item.length !== 5 || typeof item[4] !== 'string') {
			throw ('invalid settingsBio: requires array: [string, any, string, string, string]: ' + item[4]);
		}
	...
	add(item) {
		...
		this[item[4] + '_internal'] = new SettingBio(item[4], item[1], item[2]);
	...
	setLangLfm(i) {
		...
		if (panelBio.serverBio) {
			serverBio.setLangLfm(this.langLfm);
			panelBio.callServer(2, pptBio.focus, '', 1);
	...
	open() {
		const ok_callback = (new_ppt, new_cfg, type, new_dialogWindow) => {
			...
			if (new_dialogWindow) pptBio.set('Biography Dialog Box', new_dialogWindow);
		...
		const dialogWindow = pptBio.get('Biography Dialog Box', JSON.stringify({
			w: 85,
			h: 60,
			def_w: 85,
			def_h: 60,
		...
		popUpBoxBio.config(JSON.stringify(pptBio), JSON.stringify(cfg), dialogWindow, ok_callback, this.lang.ix, $Bio.folder(cfg.photoRecycler));
	}
	...
	setTag(i, handles) {
		switch (true) {
			case i && i < 12:
				...
				break;
			case i == 12: {
				const cur = $Bio.jsonParse(pptBio.get('Biography Dialog Box'), {});
				pptBio.set('Biography Dialog Box', JSON.stringify({
					w: cur.w,
					h: cur.h,
					def_w: 85,
					def_h: 60,
			...
			case i == 13:
				if (this.tagEnabled10 && this.tagEnabled11 < 7) tagBio.check(handles);
	...
	parse() {
		...
		this.tagEnabled11 = $Bio.clamp(this.tagEnabled11, 1, 6);
	...
	preview(n, tfAll, excludeStream, sFind, sReplace, artistView, trackReview) {
		...
		if (excludeStream) {
			const handle = $Bio.handle(pptBio.focus, true);
			if (!handle) return;
		...
		return panelBio.cleanPth(n, false);
	}
	...
	checkCfg() {
		...
		if ($Bio.file(updated_from_orig_ini) || !$Bio.file(orig_ini)) { // check if already updated from orig_ini
		...
		this.createCfg(settingsBio);
		$Bio.save(updated_from_orig_ini, 'true');
		const dblClick = utils.ReadINI(orig_ini, 'MISCELLANEOUS', 'Mouse Left Button Click: Map To Double-Click');
		pptBio.dblClickToggle = dblClick === '0' ? false : dblClick === '1' ? true : false;
	}

	createCfg(settingsBio) {
		settingsBio.forEach(v => {
			this.cfg[v[4]] = v[1];
		});
...
let h = '';
let settingsBio = [
		['Album Review [Allmusic] Auto-Fetch', true, 'boolean', h = 'AUTO-FETCH', 'dlAmRev'], // setting[0] & settingsBio[3] are only needed to update from orig_ini
