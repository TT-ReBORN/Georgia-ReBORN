class TaggerBio {
	constructor() {
		...
		this.arr2 = [];
		this.simList = [];
		const replacer = pptBio.replace.replace(/>/g, '|').split('|');
	...
	check(handles) {
		...
		const lfm_similar_search_done = (res1, res2, p_done, p_handles) => {
			rec++;
			if (!timerBio.sim2.id) timerBio.sim2.id = setTimeout(() => {
				writeSent = true;
	...
	lfmTidy(n, artist, album) {
		...
		const match = (v, artist, album) => {
			v = v.toLowerCase();
			if (v == artist.toLowerCase() || v == $Bio.removeDiacritics(artist).toLowerCase() || v == album.toLowerCase()) return true;
		}
		if (pptBio.cleanNo) n.forEach((v, i) => {
		...
		if (pptBio.stripNames) n = n.filter(v => !match(v, artist, album));
		if (pptBio.runReplace) n.forEach((v, i) => {
	...
	write(handles, notify) {
		...
		const cue = [];
		const locale = [];
		const radioStream = notify && panelBio.isRadio(pptBio.focus);
		const rem = [];
		const similarArtists = [];
		const tags = [];
		const tf_artist = FbTitleFormat(`$upper(${cfg.tf.artist})`);
		...
		for (let i = 0; i < handles.Count; i++) {
			...
			if (artist != cur_artist) {
				...
				if (cfg.tagEnabled7 || cfg.tagEnabled8 || cfg.tagEnabled9 || cfg.tagEnabled10 && cfg.tagEnabled11 < 7) {
					...
					artListeners[i] = '';
					locale[i] = '';
					const lfmBio = panelBio.cleanPth(cfg.pth.foLfmBio, !radioStream ? handles[i] : pptBio.focus, !radioStream ? 'tag' : 'notifyRadioStream') + $Bio.clean(artist) + cfg.suffix.foLfmBio + '.txt';
					if ($Bio.file(lfmBio)) {
						const lBio = $Bio.open(lfmBio);
						if (cfg.tagEnabled7) {
						...
						if (cfg.tagEnabled10 && cfg.tagEnabled11 < 7) {
							...
							if (similarArtists[i]) $Bio.take(similarArtists[i], cfg.tagEnabled11);
						}
					}
				}
				...
				if (cfg.tagEnabled4) {
					artGenre_am[i] = '';
					const amBio = panelBio.cleanPth(cfg.pth.foAmBio, !radioStream ? handles[i] : pptBio.focus, !radioStream ? 'tag' : 'notifyRadioStream') + $Bio.clean(artist) + cfg.suffix.foAmBio + '.txt';
					if ($Bio.file(amBio)) {
			...
			if (albumArtist + album != cur_albumArtist + cur_album) {
				...
				if (cfg.tagEnabled0 || cfg.tagEnabled1 || cfg.tagEnabled2 || cfg.tagEnabled3) {
					...
					amRating[i] = '';
					amThemes[i] = '';
					const amRev = panelBio.cleanPth(cfg.pth.foAmRev, handles[i], 'tag') + $Bio.clean(albumArtist) + ' - ' + $Bio.clean(album) + cfg.suffix.foAmRev + '.txt';
					if ($Bio.file(amRev)) {
				...
				if (cfg.tagEnabled5 || cfg.tagEnabled6) {
					albTags[i] = '';
					albListeners[i] = '';
					const lfmRev = panelBio.cleanPth(cfg.pth.foLfmRev, handles[i], 'tag') + $Bio.clean(albumArtist) + ' - ' + $Bio.clean(album) + cfg.suffix.foLfmRev + '.txt';
					if ($Bio.file(lfmRev)) {
		...
		if (handles.Count && tags.length && notify) {
			...
			window.NotifyOthers('biographyTags', {
				handle: handles[0],
				selectionMode: !pptBio.focus ? 'Prefer nowplaying' : 'Follow selected track (playlist)',
				tags: tags[0]
			});
		}
		if (notify) return;
