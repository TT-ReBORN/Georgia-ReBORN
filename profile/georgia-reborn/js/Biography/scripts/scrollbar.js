class ScrollbarBio {
	constructor() {
		...
		this.narrow = {
			show: pptBio.sbarShow == 1 ? true : false,
			x: 0
		}
	...
	move(p_x, p_y) {
		...
		if (this.type != 'film' && x >= 0 && x <= this.w && y >= 0 && y <= this.h) {
			...
			if (pptBio.sbarShow == 1 && this.scrollbar.zone != this.scrollbar.cur_zone) {
				butBio.setScrollBtnsHide(!this.scrollbar.zone || this.scrollable_lines < 1 || pptBio.img_only, this.type);
				this.scrollbar.cur_zone = this.scrollbar.zone;
			}
		...
		if (this.type != 'film') {
			if (x < 0 || x > this.w || y > this.bar.y + this.bar.h || y < this.bar.y || butBio.Dn) this.hover = false;
			else this.hover = true;
			if (!this.bar.timerBio && (this.hover != this.cur_hover || this.active != this.cur_active)) {
				this.init = false;
				this.transitionPaint();
				this.cur_active = this.active;
			}
