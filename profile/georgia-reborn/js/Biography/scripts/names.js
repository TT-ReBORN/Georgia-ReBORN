class Names {
	...
	albumArtist(focus, ignoreLock) {
		return $Bio.eval('[$trim(' + cfg.tf.albumArtist + ')]', focus, ignoreLock);
	}
	...
	artist(focus, ignoreLock) {
		return $Bio.eval('[$trim(' + cfg.tf.artist + ')]', focus, ignoreLock);
	}
	...
	title(focus, ignoreLock) {
		let n = $Bio.eval('[$trim(' + cfg.tf.title + ')]', focus, ignoreLock);
		if (cfg.local && panelBio.isRadio(focus)) {
