class ServerBio {
	constructor() {
		...
		this.lfm = {
			serverBio: cfg.langLfm.toLowerCase()
		};
		this.url = {
		...
		this.lfm.def_EN = this.lfm.serverBio == 'www.last.fm';
		this.lfm.fallback = cfg.langLfmFallback && !this.lfm.def_EN;

		this.on_playback_new_track = $Bio.debounce(() => {
			this.download(false, {
				ix: 0,
				focus: false,
				arr: []
	...
	checkNotFound() {
		if (!$Bio.file(this.notFound)) $Bio.save(this.notFound, JSON.stringify([{
			'name': 'update',
			'time': Date.now()
		}], null, 3), true);
		let m = $Bio.jsonParse(this.notFound, false, 'file');
		if (!$Bio.isArray(m)) {
			m = [{
				'name': 'update',
				'time': Date.now()
			}];
			$Bio.save(this.notFound, JSON.stringify(m, null, 3), true);
		}
		if (m[0].name != 'update') {
			m.unshift({
				'name': 'update',
	...
	checkTrack(tr) {
		...
		if (cfg.dlLfmRev && !track_done) {
			if (pptBio.inclTrackRev) this.getTrack(tr);
	...
	createImgDlFile() {
		...
		if (!$Bio.file(n) || fsoBio.GetFile(n).Size == '696') {
			...
			$Bio.save(n, dl_im, false);
		}
	}

	done(f, exp) {
		if (!$Bio.file(this.notFound)) return false;
	...
	expired(f, exp, f_done, langCheck, type) {
		...
		if (!$Bio.file(f)) return true;
		return Date.now() - $Bio.lastModified(f) > exp;
	}
	...
	getBio(force, art, type) {
		switch (type) {
			case 0: {
				...
				if (!artist_done) {
					...
					if (cfg.lfmSim && stndBio) {
						const fo_sim = panelBio.cleanPth(cfg.pth.foLfmSim, pptBio.focus, 'serverBio');
						const pth_sim = fo_sim + $Bio.clean(this.artist) + ' And Similar Artists.json';
						let len = 0;
						let valid = false;
						if ($Bio.file(pth_sim)) {
							const list = $Bio.jsonParse(pth_sim, false, 'file');
							if (list) {
								valid = $Bio.objHasOwnProperty(list[0], 'name');
								len = list.length;
							}
						}
					...
					if (stndBio && cfg.photoLimit && !panelBio.lock) { // purge imgToRecycle
						...
						while (j--) {
							if (this.imgToRecycle[j].a != name.artist(true) && this.imgToRecycle[j].a != name.artist(false)) {
								...
								if (!$Bio.file(this.imgToRecycle[j].p)) this.imgToRecycle.splice(j, 1)
							}
						}
			...
			case 1: {
				...
				const am_bio = panelBio.getPth('bio', art.focus, this.artist, '', stndBio, false, $Bio.clean(this.artist), '', '', 'foAmBio', true, true);
	...
	getCover(force, alb) { // stndAlb
		const handle = $Bio.handle(alb.focus, true);
		if (!handle) return;
		...
		switch (sw) {
			case 1: { // cover
				const cov = panelBio.getPth('cov', alb.focus, 'serverBio');
	...
	getRev(force, art, alb, onlyForceLfm) {
		...
		if (cfg.dlAmRev && !onlyForceLfm) {
			...
			if (rev_upd) {
				let amRev = $Bio.open(am_rev.pth);
				rev_upd = !amRev || (!amRev.includes('Genre: ') || !amRev.includes('Review by ') && Date.now() - $Bio.lastModified($Bio.file(am_rev.pth)) < this.exp) && !amRev.includes('Custom Review');
			}
			let dn_type = '';
	...
	getTrack(tr) {
		...
		const lfm_tracks = panelBio.getPth('track', pptBio.focus, tr.artist, 'Track Reviews', '', '', $Bio.clean(tr.artist), '', 'Track Reviews', 'foLfmRev', true, true);
		const text = $Bio.jsonParse(lfm_tracks.pth, false, 'file');
	...
	match(p_a, p_release, list, type) {
		const rel_m = [];
		const art_m = [];
		let a = $Bio.removeDiacritics(this.tidy(p_a, true));
		let i = 0;
		let rel = $Bio.removeDiacritics(this.tidy(p_release, true));
		for (i = 0; i < list.length; i++) {
			rel_m[i] = $Bio.removeDiacritics(this.tidy(list[i].title || 'N/A', true));
			art_m[i] =  $Bio.removeDiacritics(this.tidy(list[i].artist || 'N/A', true));
	...
	processAmBio(responseText, artist, album, title, fo_bio, pth_bio, pth_rev) {
		...
		try {
			...
			$Bio.htmlParse(div.getElementsByTagName('h3'), 'className', 'headline', v => biographyAuthor = serverBio.format(v.innerHTML));
	...
	setImgRecycler(n) {
		if (!$Bio.isArray(this.imgToRecycle)) {
			this.imgToRecycle = [];
			n = true;
		}
		if (n) $Bio.save(this.bioCache, JSON.stringify(this.imgToRecycle, null, 3), true);
	}

	setLangLfm(lang) {
		if (lang) this.lfm.serverBio = lang.toLowerCase();
		this.lfm.serverBio = this.lfm.serverBio == 'en' ? 'www.last.fm' : 'www.last.fm/' + this.lfm.serverBio;
		this.lfm.def_EN = this.lfm.serverBio == 'www.last.fm';
		this.lfm.fallback = cfg.langLfmFallback && !this.lfm.def_EN;
	}
	...
	updateNotFound(f) {
		if (!$Bio.file(this.notFound)) return;
		const m = $Bio.jsonParse(this.notFound, false, 'file');
		const n = Date.now();
		let k = m.length;
		for (k = 0; k < m.length; k++)
	...
	onStateChange() {
		if (this.xmlhttp != null && this.func != null)
			if (this.xmlhttp.readyState == 4) {
				clearTimeout(this.timerBio);
				this.timerBio = null;
				if (this.xmlhttp.status == 200) this.func();
				else $Bio.trace('allmusic album review / biography: ' + serverBio.album + ' / ' + serverBio.albumArtist + ': not found' + ' Status error: ' + this.xmlhttp.status, true);
			}
	}
		...
		switch (this.sw) {
			case 0:
				try {
					...
					list = serverBio.parseAmSearch(div, 'performers', 'song');
					i = serverBio.match(this.artist, this.title, list, 'title');
					if (i != -1) {
				...
				break;
			case 1:
				serverBio.processAmBio(this.xmlhttp.responseText, this.artist, '', this.title, this.fo_bio, this.pth_bio, '');
				break;
		}
	}
}
	...
	onStateChange() {
		if (this.xmlhttp != null && this.func != null)
			if (this.xmlhttp.readyState == 4) {
				...
				this.timerBio = null;
				if (this.xmlhttp.status == 200) this.func();
				else $Bio.trace('allmusic album review / biography: ' + this.album + ' / ' + this.albumArtist + ': not found' + ' Status error: ' + this.xmlhttp.status, true);
			}
	}
	...
	analyse() {
		...
		switch (this.sw) {
			case 0:
				try {
					div.innerHTML = this.xmlhttp.responseText;
					list = serverBio.parseAmSearch(div, 'artist', 'album');
					i = serverBio.match(this.albumArtist, this.album, list, 'rev');
					if (i != -1) {
					...
					$Bio.trace('allmusic album review: ' + this.album + ' / ' + this.albumArtist + ': not found', true);
				} catch (e) {
					serverBio.getBio(this.force, this.art, 1);
					serverBio.updateNotFound('Bio ' + cfg.partialMatch + ' ' + this.pth_rev);
					serverBio.updateNotFound('Rev ' + cfg.partialMatch + ' ' + this.pth_rev);
			...
			case 1:
				try {
					...
					$Bio.htmlParse(div.getElementsByTagName('h3'), 'className', 'review-author headline', v => reviewAuthor = serverBio.format(v.innerHTML));
			...
			case 2:
				docBio.close();
				serverBio.processAmBio(this.xmlhttp.responseText, this.artist, this.album, '', this.fo_bio, this.pth_bio, this.pth_rev);
				break;
		}
	}
}
...
class DldLastfmBio {
	...
	analyse(saveOnly) {
		...
		if (!saveOnly) {
			...
			switch (this.searchBio) {
				case 0: {
					...
					if (this.simArtists.length) this.simArtists = '\r\n\r\n' + serverBio.similar[cfg.lang.ix] + this.simArtists.join('\u200b, ');
		...
		if ((!this.con.length || this.con.length < 45 && noWiki(this.con)) && serverBio.lfm.fallback && !this.retry) {
			this.retry = true;
			this.searchBio = 1;
			return this.search(this.artist, this.fo_bio, this.pth_bio);
		}
...
class DldArtImages {
	img_exp(p_dl_ar, imgFolder, ex) {
		...
		if (newImgNo > 0) return [newImgNo, 0, allFiles];
			...
			if (cfg.photoLimit) {
				...
				if (remove > 0) {
					for (let j = 0; j < remove; j++) serverBio.imgToRecycle.push({
						a: p_dl_ar,
						p: imgExisting[j].p
					});
	...
	run(dl_ar, force, art, p_stndBio, p_supCache) {
		if (!$Bio.file(fb.ProfilePath + 'yttm\\foo_lastfm_img.vbs')) return;
		let img_folder = p_stndBio ? panelBio.cleanPth(cfg.pth.foImgArt, art.focus, 'serverBio') : panelBio.cleanPth(cfg.remap.foImgArt, art.focus, 'remap', dl_ar, '', 1);
...
class LfmArtImg {
	...
	search(p_dl_ar, p_img_folder, p_getNo, p_autoAdd, p_allFiles, p_imgExisting, force) {
		...
		this.func = null;
		this.xmlhttp = new ActiveXObject('Microsoft.XMLHTTP');
		const URL = 'https://' + (!this.retry ? serverBio.lfm.serverBio : 'www.last.fm') + '/music/' + encodeURIComponent(this.dl_ar) + '/+images';
		this.func = this.analyse;
		this.xmlhttp.open('GET', URL);
		this.xmlhttp.onreadystatechange = this.ready_callback;
		if (force) this.xmlhttp.setRequestHeader('If-Modified-Since', 'Thu, 01 Jan 1970 00:00:00 GMT');
	...
	analyse() {
		...
		let links = [];
		if (!list) {
			if (serverBio.lfm.fallback && !this.retry) {
				this.retry = true;
				docBio.close();
		...
		if (links.length) {
			...
			if ($Bio.folder(this.img_folder)) {
				if (this.autoAdd && cfg.photoLimit) {
					...
					if (remove > 0) {
						for (k = 0; k < remove; k++) serverBio.imgToRecycle.push({
							a: a,
				...
				if (this.autoAdd) {
					$Bio.take(links, this.getNo).forEach(v => $Bio.run('cscript //nologo "' + fb.ProfilePath + 'yttm\\foo_lastfm_img.vbs" "' + v + '" "' + this.img_folder + a + '_' + v.substring(v.lastIndexOf('/') + 1) + '.jpg' + '"', 0));
				} else {
					let c = 0;
					$Bio.take(links, cfg.photoNum).some(v => {
						...
						if (!this.allFiles.includes(imPth)) {
							$Bio.run('cscript //nologo "' + fb.ProfilePath + 'yttm\\foo_lastfm_img.vbs" "' + v + '" "' + imPth + '"', 0);
							c++;
							return c == this.getNo;
						}
					});
...
class LfmAlbum {
	...
	search(p_alb_artist, p_album, p_rev, p_fo, p_pth, p_albm, force, p_rev_img) {
		...
		if (!this.getStats && this.rev || !this.rev) {
			...
			if (this.rev && !serverBio.lfm.def_EN && !this.retry) URL += '&lang=' + cfg.langLfm.toLowerCase();
		...
		} else URL = 'https://' + serverBio.lfm.serverBio + '/music/' + encodeURIComponent(this.albumArtist) + '/' + encodeURIComponent(this.album.replace(/\+/g, '%2B'));
		this.func = null;
		this.xmlhttp = new ActiveXObject('Microsoft.XMLHTTP');
		this.func = this.analyse;
		this.xmlhttp.open('GET', URL);
	...
	analyse() {
		if (!this.getStats && this.rev) {
			...
			$Bio.htmlParse(div.getElementsByTagName('dt'), 'className', 'catalogue-metadata-heading', v => {
				if (j) {
					rd_n = $Bio.titlecase(v.innerText.trim());
					return true;
				}
				j++;
			});
			...
			$Bio.buildPth(this.fo);
			$Bio.run('cscript //nologo "' + fb.ProfilePath + 'yttm\\foo_lastfm_img.vbs" "' + link + '" "' + this.pth + link.slice(-4) + '"', 0);
		}
	}
}
...
class LfmTrack {
	...
	search(p_artist, p_track, p_fo, p_pth, p_force) {
		...
		if (!this.lfm_done) {
			if (!this.getStats) {
				URL = serverBio.url.lfm;
				if (!serverBio.lfm.def_EN && !this.retry) URL += '&lang=' + cfg.langLfm.toLowerCase();
				...
				URL = 'https://' + serverBio.lfm.serverBio + '/music/' + encodeURIComponent(this.artist) + '/_/' + encodeURIComponent(this.track);
			}
		} else {
		...
		if (!this.lfm_done) {
			if (!this.getStats) {
				...
				if (!this.wiki) {
					...
					if (!this.lfm_done && (cfg.langLfm == 'EN' || serverBio.lfm.fallback)) {
						this.lfm_done = true;
						return this.search(this.artist, this.track, this.fo, this.pth, this.force);
					}
				...
				if (!cfg.lang.ix) $Bio.htmlParse(div.getElementsByTagName('p'), 'className', 'more-link-fullwidth', v => {
			...
			if (!this.getIDs) {
				...
				this.text.ids = {}
				$Bio.htmlParse(div.getElementsByTagName('a'), false, false, v => {
					if (v.parentNode && v.parentNode.nodeName == 'LI' && v.href.includes('/facts/')) this.text.ids[serverBio.tidy(v.innerText)] = v.href.replace('about:/', '');
				});
	...
	revSave(ret) {
		...
		$Bio.buildPth(this.fo);
		$Bio.save(this.pth, JSON.stringify(this.text, null, 3), true);
