    ...
    this.draw = gr => {
        if (this.scrollable_lines > 0) {
            switch (ui.sbarType) {
                ...
                case 1:
                    if (!pptDefault.sbarCol) {
                        if (libraryProps.showScrollbar && sbar.w === scaleForDisplay(12) || !pref.autoHideScrollbar_Library && sbar.w === scaleForDisplay(12)) {
                            const thumbColors = [
                                // normal
                                pref.whiteTheme ? RGBA(200, 200, 200, alpha) :
                                pref.blackTheme ? RGBA(100, 100, 100, alpha) :
                                pref.blueTheme ? RGBA(10, 135, 225, alpha) :
                                pref.darkblueTheme ? RGBA(27, 55, 90, alpha) :
                                pref.redTheme ? RGBA(145, 25, 25, alpha) :
                                pref.creamTheme ? RGBA(200, 200, 200, alpha) :
                                pref.nblueTheme ? RGBA(0, 200, 255, alpha) :
                                pref.ngreenTheme ? RGBA(0, 200, 0, alpha) :
                                pref.nredTheme ? RGBA(229, 7, 44, alpha) :
                                pref.ngoldTheme ? RGBA(254, 204, 3, alpha) : '',
                                // hover
                                pref.whiteTheme ? RGBA(120, 120, 120, alpha) :
                                pref.blackTheme ? RGBA(160, 160, 160, alpha) :
                                pref.blueTheme ? RGBA(242, 230, 170, alpha) :
                                pref.darkblueTheme ? RGBA(255, 202, 128, alpha) :
                                pref.redTheme ? RGBA(245, 212, 165, alpha) :
                                pref.creamTheme ? RGBA(120, 170, 130, alpha) :
                                pref.nblueTheme ? RGBA(0, 238, 255, alpha) :
                                pref.ngreenTheme ? RGBA(0, 255, 0, alpha) :
                                pref.nredTheme ? RGBA(255, 0, 0, alpha) :
                                pref.ngoldTheme ? RGBA(255, 242, 3, alpha) : '',
                                // drag
                                pref.whiteTheme ? RGBA(120, 120, 120, alpha) :
                                pref.blackTheme ? RGBA(160, 160, 160, alpha) :
                                pref.blueTheme ? RGBA(242, 230, 170, alpha) :
                        ...
                        gr.FillSolidRect(this.x, this.y + bar_y, this.w, bar_ht, RGBA(255, 255, 255, 255) &
                            (!hover && !b_is_dragging ?
...
function panel_operations() {
    ...
    var grps = [],
        i = 0;
    // vjs_stnd = window.GetProperty("ADV.Scrollbar Height Always Full", true);
    // js_stnd = !js_stnd ? 2 : 0;
    ...
    for (i = ppt_l; i < ppt_l + 93; i++) {
        nm = window.GetProperty(prefix + "View " + padNumber(i, 0) + ": Name // Pattern");
        if (nm && nm != " // ") view_ppt.push(window.GetProperty(prefix + "View " + padNumber(i, 0) + ": Name // Pattern"));
    }
    if (!window.GetProperty("SYSTEM.View Update", false)) {
        i = view_ppt.length + 1;
        window.SetProperty(prefix + "View " + padNumber(i, 0) + ": Name // Pattern", null);
        view_ppt.push(window.GetProperty(prefix + "View " + padNumber(i, 0) + ": Name // Pattern", "View by Path // $directory_path(%path%)|%filename_ext%$nodisplay{%subsong%}"));
...
function LibraryTree() {
            ...
            nd[j] = s.gr(sz, sz, true, (/** @type {GdiGraphics} */ g) => {
                ...
                for (let i = 0; i < 4; i++)
                    ...
                    if ((pref.whiteTheme
                      ...
                      || pref.nredTheme
                      || pref.ngoldTheme)
                      && pref.libraryDesign === 'library_traditional') {
                        g.FillSolidRect(x, y, sz, sz, ui.col.iconPlusbg);
                    }
                    g.SetSmoothingMode(SmoothingMode.AntiAlias);

                    if (pref.libraryDesign === 'library_modern') {
    ...
    this.draw = gr => {
        try {
            ...
            for (var i = start_row; i < last_row; i++) {
                ...
                if (item.sel && (ui.col.bgSel !== 0 || col.primary !== 0)) {
                    ...
                    if (pref.libraryDesign === 'library_traditional') {
                        ...
                        if (libraryProps.fullLine) sel_x = ui.x; sel_w = ui.x + sbar.tree_w - sel_x + scaleForDisplay(20);
                        if (!libraryProps.fullLine) sel_x = ui.x; sel_w = ui.x + sbar.tree_w - sel_x + scaleForDisplay(20);
                    }
                    if (!tt.Text || tt.Text || m_i != i && tt.Text) {
                        bgColor = col.primary;
                        if (pref.libraryDesign === 'library_traditional') {
                            gr.FillSolidRect(sel_x, item_y, sel_w, ui.row_h,
                        ...
                        if (pref.libraryDesign === 'library_modern') {
                            gr.FillSolidRect(sel_x, item_y, sel_w, ui.row_h,
                                ...
                                pref.blueTheme ? RGB(10, 130, 220) :
                                pref.darkblueTheme ? RGB(24, 50, 82) :
                                pref.redTheme ? RGB(135, 25, 25) :
                                pref.creamTheme ? RGB(120, 170, 130) :
                                pref.nblueTheme || pref.ngreenTheme || pref.nredTheme || pref.ngoldTheme ? RGB(30, 30, 30) : ''
                            );
                            gr.FillSolidRect(sel_x, item_y, scaleForDisplay(8), ui.row_h,
            ...
            for (i = start_row; i < last_row; i++) {
                ...
                if (!tt.Text || tt.Text) {
                    if (m_i == i) { // hovered
                        ...
                        if (pref.libraryDesign === 'library_modern') {
                            ...
                            gr.DrawRect(sel_x, item_y - 1, sel_w, ui.row_h + 1, 1,
                                pref.whiteTheme ? RGB(230, 230, 230) :
                ...
                item.w = item_w;
                if (!libraryProps.fullLine && pref.libraryDesign === 'library_modern') {
    ...
    this.get_ix = function(x, y, simple, type) {
        ...
        //x -= ui.x;
        if (pref.libraryDesign === 'library_traditional') { x -= ui.x; } else if (pref.libraryDesign === 'library_modern') { x -= ui.x - (ui.margin + ui.icon_w); }
...
class LibraryPanel {
    ...
    on_size(x, y, width, height) {
        ...
        this.h = height;
        ui.x = x;
        ui.y = y + scaleForDisplay(12);
        ui.w = width;
        ui.h = height - g_properties.row_h - scaleForDisplay(12);
        ...
        if (pptDefault.sbarShow) {
            switch (ui.sbarType) {
                case 2:
                    ...
                    break;
                default:
                    this.btns.scrollUp = new Btn(b_x - scaleForDisplay(2), scrollUp_y, ui.scr_but_w, ui.but_h, 1, null, null, scrollBtns.lineUp.stateImgs, "", sbar.scrollable_lines < 1, () => sbar.but(1), "", "", false, "scrollUp");
                    this.btns.scrollDn = new Btn(b_x - scaleForDisplay(2), scrollDn_y, ui.scr_but_w, ui.but_h, 2, null, null, scrollBtns.lineDown.stateImgs, "", sbar.scrollable_lines < 1, () => sbar.but(-1), "", "", false, "scrollDn");
                    break;
            }
        }
    ...
    on_mouse_move(x, y, m) {if (p.m_x == x && p.m_y == y) return; if (libraryProps.searchMode || libraryProps.showScrollbar) but.move(x, y); if (libraryProps.searchMode) sL.move(x, y); library_tree.move(x, y); library_tree.dragDrop(x, y); sbar.move(x, y); p.m_x = x; p.m_y = y;
        ...
        if (pref.autoHideScrollbar_Library) {
            ...
            if ((scrollBar_hitarea_x - scaleForDisplay(20) * trace_pad <= x) && (x <= scrollBar_hitarea_x - scaleForDisplay(20) + scrollBar_hitarea_w + trace_pad) &&
                (scrollBar_hitarea_y + (is_4k ? 140 : 80) <= y) && (y <= scrollBar_hitarea_y + scrollBar_hitarea_h + (is_4k ? 90 : 50))) {
            ...
            } else if ((!sbar.isDragging() && scrollBar_hitarea_x - scaleForDisplay(30) * trace_pad <= x) && (x <= scrollBar_hitarea_x - scaleForDisplay(30) + scrollBar_hitarea_w + trace_pad) &&
