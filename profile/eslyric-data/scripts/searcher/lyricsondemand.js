...
export function getConfig(cfg) {
	cfg.name = 'LyricsOnDemand (Unsynced)';
	cfg.version = '0.1';
	cfg.author = 'TT';
...
export function getLyrics(meta, man) {
	...
	const artist = Clean(meta.artist).replaceAll('the ', '').replace(/ /g, ''); // LyricsOnDemand formatting
	const artistLetter = artist.charAt(0);
	const title = Clean(meta.title).replace(/ /g, ''); // LyricsOnDemand formatting
	const url = `https://www.lyricsondemand.com/${artistLetter}/${artist}lyrics/${title}lyrics.html`;
...
function findLyrics(rootElement) {
	...
	for (const attribute of rootElement.attributes || []) {
		if (attribute.key === 'class' && attribute.value === 'lcontent') {
			lyricContainerElements.push(rootElement);
			return true;
		}
	}

	// Check when the website has additional <div id="sbmtlyr"> container
	for (const attribute of rootElement.attributes || []) {
		if (attribute.key === 'id' && attribute.value === 'sbmtlyr') {
			lyricContainerElements.push(rootElement);
			return true;
		}
	}
...
function parseLyrics(element, lyricText) {
	const Clean = (rawString) => rawString.trim()
		...
		.replace(/,/g, ', ').replace(/, /g, ', ') // LyricsOnDemand formatting
		.replace(/./g, '. ').replace(/. /g, '. '); // LyricsOnDemand formatting
	...
	if (tag === 'br') { // LyricsOnDemand formatting
		return lyricText;
	}
